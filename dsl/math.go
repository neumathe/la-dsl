...
func matrixAddInt(a, b *MatrixInt) (*MatrixInt, error) {
	if a.R != b.R || a.C != b.C {
		return nil, errors.New("matrix add: shape mismatch")
	}
...
func matrixSubInt(a, b *MatrixInt) (*MatrixInt, error) {
	if a.R != b.R || a.C != b.C {
		return nil, errors.New("matrix sub: shape mismatch")
	}
	res := NewMatrixInt(a.R, a.C)
	for i := 0; i < a.R; i++ {
		for j := 0; j < a.C; j++ {
...
func scalarMulMatrixInt(k int64, a *MatrixInt) *MatrixInt {
	res := NewMatrixInt(a.R, a.C)
	for i := 0; i < a.R; i++ {
		for j := 0; j < a.C; j++ {
			res.A[i][j] = k * a.A[i][j]
