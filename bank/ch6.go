...
func buildChapter6_6() dsl.Problem {
	...
	return dsl.Problem{
		...
		Version: "bank-v1",
		Title: fmt.Sprintf(
			`若二次型 $f(x_1,x_2,x_3)={{expr}}$ 经正交变换 $x=Py$ 化为标准型 $f=\lambda_1 y_1^2+\lambda_2 y_2^2+\lambda_3 y_3^2$（其中 $\lambda_1={{lambda1}}$，$\lambda_2={{lambda2}}$，$\lambda_3={{lambda3}}$），则参数 $t$={{blank:%s}}，正交变换矩阵 $P$ 的 9 个元素（按行自上而下、从左到右；每一列允许整体乘以非零整数不影响判分）：%s`,
			ids[0],
...
**步骤 2：** 利用迹等式求 $t$。正交相似变换 $P^\top SP=\mathrm{diag}(\lambda_1,\lambda_2,\lambda_3)$ 不改变矩阵的迹，故 $\mathrm{tr}(S)=\lambda_1+\lambda_2+\lambda_3$。已知标准型系数 $\lambda_1={{lambda1}}$，$\lambda_2={{lambda2}}$，$\lambda_3={{lambda3}}$，故 $\mathrm{tr}(S)=\lambda_1+\lambda_2+\lambda_3$。设 $S$ 中含 $t$ 的对角元为 $S_{kk}$，则 $\mathrm{tr}(S)=S_{11}+S_{22}+S_{33}$（其中一项含 $t$）。令该式等于 $\lambda_1+\lambda_2+\lambda_3$，代入数值解方程得 $t={{expr:param_t(S)}}$。
