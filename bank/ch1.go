...
func buildChapter1_4() dsl.Problem {
	...
	return dsl.Problem{
		...
		Answer:  dsl.AnswerSchema{FieldDefs: []dsl.AnswerFieldDef{{ID: id, Expr: "d"}}},
		Meta: map[string]interface{}{
			"solution_zh": `**解题思路：** 这是一个"主对角线元素全为 $a$、其余所有元素全为 $b$"的等对角线行列式（并非三对角矩阵）。利用其特殊结构，可用特征值法直接给出闭式结果。

**步骤 1：** 识别矩阵结构。该 $n$ 阶矩阵可写成 $A=(a-b)I + bJ$，其中 $I$ 为单位阵、$J$ 为全 $1$ 矩阵：
- 主对角线元素全为 $a = a_{11} = {{expr:mget(A,1,1)}}$（验证：$a_{22}={{expr:mget(A,2,2)}}, a_{33}={{expr:mget(A,3,3)}}$ 等均等于 ${{expr:mget(A,1,1)}}$）
- 其余所有元素全为 $b = a_{12} = {{expr:mget(A,1,2)}}$（验证：$a_{23}={{expr:mget(A,2,3)}}, a_{34}={{expr:mget(A,3,4)}}$ 等均等于 ${{expr:mget(A,1,2)}}$）

**步骤 2：** 求特征值。每一行元素之和均为 $a+(n-1)b$，故全 $1$ 向量 $(1,1,\cdots,1)^T$ 是特征向量，对应特征值
$$\lambda_1 = a+(n-1)b$$
而任何分量之和为 $0$ 的向量 $x$ 满足 $Ax=(a-b)x$，故 $(a-b)$ 是特征值，其重数为 $n-1$：
$$\lambda_2 = a-b\quad(\text{重数 } n-1)$$

**步骤 3：** 行列式等于全部特征值之积：
$$D_n = \lambda_1\cdot\lambda_2^{\,n-1} = (a-b)^{n-1}\bigl(a+(n-1)b\bigr)$$

**步骤 4：** 代入 $n=7$，得
$$D_7 = (a-b)^{6}\,(a+6b)$$
将 $a={{expr:mget(A,1,1)}}$、$b={{expr:mget(A,1,2)}}$ 代入，最终得
$$D_7 = {{d}}$$`,
		},
	}
}
